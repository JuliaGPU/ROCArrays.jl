...
#
# this allocator sits between ROCArray constructors
# and the actual memory allocation in HSARuntime.Mem
#
...
const tracing = parse(Bool, get(ENV, "ROCARRAYS_TRACE_POOL", "false"))

function __init_memory__()
  create_pools(30) # up to 512 MiB

  managed = parse(Bool, get(ENV, "ROCARRAYS_MANAGED_POOL", "true"))
  if managed
    delay = MIN_DELAY
    @async begin
      while true
  ...
  verbose = haskey(ENV, "ROCARRAYS_MANAGED_POOL")
  if verbose
    atexit(()->begin
      Core.println("""
      ...
      if full
        ...
        for (pid, pl) in enumerate(pools_avail)
          for buf in pl
            ...
            stats.roc_time += Base.@elapsed Mem.free(buf)
    ...
    lock(pool_lock) do
      ...
      if pooling && !isempty(avail)
        ...
        try
          ...
          stats.roc_time += Base.@elapsed begin
          ...
          ex isa HSAError && ex.code == HSARuntime.HSA_STATUS_OUT_OF_RESOURCES || rethrow()
...
macro time(ex)
    quote
        ...
        local gpu_gc_time = gpu_mem_stats1.roc_time - gpu_mem_stats0.roc_time
        local gpu_lib_time = gpu_mem_stats1.roc_time - gpu_mem_stats0.roc_time
