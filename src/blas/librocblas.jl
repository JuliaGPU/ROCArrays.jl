...
function rocblas_get_version_string()
    buf = repeat(" ", 64)
    ccall((:rocblas_get_version_string, "librocblas"), rocblas_status_t, (Cstring, Csize_t), buf, len)
    return buf
end
...
# Level 1 BLAS
# TODO: Ensure HSAArray types match BLAS call
# TODO: Complex

function rocblas_dscal(handle, n, alpha::Cdouble, x::HSAArray, incx)
    ref_alpha = Ref(alpha)
...
function rocblas_sscal(handle, n, alpha::Cfloat, x::HSAArray, incx)
    ref_alpha = Ref(alpha)
    GC.@preserve ref_alpha begin
...
function rocblas_dcopy(handle, n, x::HSAArray, incx, y::HSAArray, incy)
...
function rocblas_scopy(handle, n, x::HSAArray, incx, y::HSAArray, incy)
...
function rocblas_ddot(handle, n, x::HSAArray, incx, y::HSAArray, incy, result)
...
function rocblas_sdot(handle, n, x::HSAArray, incx, y::HSAArray, incy, result)
...
function rocblas_dswap(handle, n, x::HSAArray, incx, y::HSAArray, incy)
    @check ccall((:rocblas_dswap, "librocblas"),
                 rocblas_status_t,
                 (rocblas_handle, rocblas_int, Ptr{Cdouble}, rocblas_int, Ptr{Cdouble}, rocblas_int),
                 handle, n, x.handle, incx, y.handle, incy)
end
function rocblas_sswap(handle, n, x::HSAArray, incx, y::HSAArray, incy)
